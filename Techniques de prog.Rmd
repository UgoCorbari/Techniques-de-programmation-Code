...
compet <- page %>% 
  html_node(".direct-headline__header") %>% 
  html_text(trim = TRUE)

# Équipe jouant à domicile
T.home <- page %>% 
  html_node(".sb-team.sb-heim .sb-vereinslink") %>% 
  html_text(trim = TRUE)

# Équipe se déplacant à l'extérieur
T.away <- page %>% 
  html_node(".sb-team.sb-gast .sb-vereinslink") %>% 
  html_text(trim = TRUE)

# Résultat du match 
result <- page %>% 
  html_node(".sb-endstand") %>%
  html_text(trim = TRUE)

# Nommbre de spectateurs présents au stade 
spec <- page %>% 
  html_node(".sb-zusatzinfos strong") %>% 
  html_text(trim = TRUE)

# Date et heure du match 
date <- page %>% 
  html_node(".sb-datum.hide-for-small") %>% 
  html_text(trim = TRUE)

# Nom du stade dans lequel se déroule le match
Stade <- page %>% 
  html_node(".sb-zusatzinfos a") %>% 
  html_text(trim = TRUE)
...
```{r}
# Extraire les détails des événements liés aux buts et passes décisives
but_details <- page %>%
  html_nodes(".sb-aktion-aktion") %>%
  html_text(trim = TRUE)

# Filtrer pour ne garder que les événements de but
but_events <- grep("But de la saison", but_details, value = TRUE)

# Extraire les buteurs et passeurs depuis les événements filtrés
buteurs <- page %>%
  html_nodes(".sb-aktion-aktion a.wichtig") %>%
  html_text(trim = TRUE)

# Initialisation des listes pour les buteurs et les passeurs
liste_buteurs <- c()
liste_passeurs <- c()

# Extraire les statistiques des événements
for (event in but_events) {
  # Extraire le buteur
  buteur_match <- sub(",.*", "", event) # Le texte avant la virgule est le buteur
  liste_buteurs <- c(liste_buteurs, buteur_match)
  
  # Extraire le passeur (si présent)
  passeur_match <- ifelse(grepl("Passe décisive:", event),
                          sub(".*Passe décisive: (.+?),.*", "\\1", event), # Passeur identifié
                          "Non spécifié") # Sinon, passeur non mentionné
  liste_passeurs <- c(liste_passeurs, passeur_match)
}

# Créer un tableau pour associer les informations
