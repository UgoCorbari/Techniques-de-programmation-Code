...
# La frise présente tous les évènements du match, changements, buts, cartons. Or dans notre cas seul les buts nous intéressent
# Filtrer uniquement les événements de buts
but_elements <- page %>%
...
for (i in seq_along(but_events)) {
  buteur_match <- sub(",.*", "", but_events[i])                                        #On extrait le(s) buteur(s) et on les ajoutes 
                                                                                      #dans la liste vide 
  passeur_match <- ifelse(grepl("Passe décisive:", but_events[i]),
                          sub(".*Passe décisive: (.+?),.*", "\\1", but_events[i]),            #Extraction des passeurs si identifiés 
